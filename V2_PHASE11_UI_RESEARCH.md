...
14. **Empty-state chip seeding from `alternateGreetings` is mostly inert today** — surfaced 2026-05-10 while smoking 4.h.2. Character-bound chats use `newChat(withCharacter:)` which seeds `firstMessage` as turn 0 immediately, exposing `alternateGreetings` as variants on that turn. The empty state — where the new chip seeding fires — therefore never installs for the common character-chat case (`chat.activeTurns` is non-empty). The chip-seeding helper is correctly pinned by Phase11ChipSeedTests but its user-facing surface is just (a) free-form chats with no character, and (b) the rare character chat without a `firstMessage`. Possible future direction: surface `alternateGreetings` as "swap opener" chips near the existing variants pill on the seeded turn 0, rather than on an empty state that almost never shows. Distinct enough from §4.6 ("empty state of a fresh chat") to warrant its own design note. Flag for the per-surface design pass after V11 chat-pane lands.
