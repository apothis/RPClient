...
**Effort: ~2 days for §7.2–§7.5 once §7.1 lands.** §7.1, §7.2 (a/b/c), §7.5 (a/b/c/d), §7.3, §7.4 (a/b) all closed 2026-05-05 — Phase 6 is feature-complete for per-character voices, plus the attribution-mode escape hatch and the per-voice preview button. Only §7.6 (heuristic refinement pass over a real chat corpus) remains.
...
From a fresh context, the pragmatic next move is **§7.6** (heuristic refinement — the load-bearing piece for "usually right" → "reliably right"). After that, **Phase 7 (V2 Full branching)** or **Phase 8 (V9 Group chats)** — both are sizeable refactors that warrant their own design rounds (PLAN.md §11).
