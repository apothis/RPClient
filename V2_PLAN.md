...
3. **Voice catalogue (sub-step 7.1c — shipped 2026-05-05).** `Sources/RPClientCore/Voice/KokoroVoiceCatalogue.swift` (in Core, not RPClientVoice — pure data with no ONNX dep, lets `RPClientCoreTests` verify it without a second test runner). Exposes `KokoroVoiceCatalogue.all: [KokoroVoice]` as a Swift literal: 54 entries with id, displayName, KokoroLanguage, KokoroGender, per-language sampleText. Download URL is derived (`https://huggingface.co/hexgrad/Kokoro-82M/resolve/main/voices/<id>.pt`); `modelDownloadURL` + `modelByteSize` (325_532_387, exact) are constants on the namespace. **Not** a Bundle resource — ships in the binary as compiled code. 13 tests covering id uniqueness, language/gender inference, filtering, URL shape, model constants. 261/261 green.
...
7. **First-run storage prompt.** New `UI/Voice/VoiceStoragePromptSheet.swift` — fired when `voiceEnabled` flips on and `Settings.voiceModelPath` is nil. Lists detected non-system mounted volumes (sorted by free space), Application Support, and a "Choose…" escape hatch. Persists choice; dismissing the sheet falls back to Application Support. Subsequent toggles do not re-prompt.
8. **Settings UI — Voice library manager.** New `UI/Settings/VoiceLibraryView.swift` rendering the persistent catalogue table, download progress bars, and the storage-location picker. Per-voice download is a one-shot URLSession download task with progress reporting; cancellable mid-download. Concurrent download cap = 2. No "download all" button; every download is opt-in per voice.
9. **Smoke test.** `./build.sh && ./run.sh`, toggle Speak replies on (triggers first-run prompt → pick the external SSD), download base model + one voice, send a message, confirm Kokoro output is materially better than the §7.0 AVKit baseline.
