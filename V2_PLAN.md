...
- **§7.4b — Queue + per-segment voice swap** — pending, lands with §7.3. Once attribution exists (decides "Sage said this line"), `Speaker` enqueues one utterance per segment with the matched voice. The §7.4a engine refactor already supports the per-segment swap pattern — Kokoro just needs to be called with different voice options per call.
...
**Effort: ~2 days for §7.2–§7.5 once §7.1 lands.** §7.1, §7.2 (a/b/c), §7.5 (a/b), §7.4a all closed 2026-05-05. Remaining: §7.4b (queue) and §7.3 (speaker attribution) — these are now coupled and land together because attribution is what produces the segment list the queue consumes. Optional §7.5c: Preview button on the entity card (now unblocked by §7.4a's per-call swap).
...
Phases 1, 2, 3 are done. Phase 6 §7.1, §7.2 (a/b/c), §7.5 (a/b), §7.4a all closed 2026-05-05. **The chat-header and Settings voice pickers are now audible** — pick a voice on the chat header, the next assistant turn narrates in that voice. 418/418 tests pass. Per-character routing still pending — entity voices write to the data model but every turn is currently one segment.

From a fresh context: pick up at **Phase 6 §7.3 + §7.4b** as a paired commit. §7.3 (speaker attribution) decides "Sage said this line" via heuristic (`^"…"` matched to most-recently-mentioned entity) or tagged (`Sage: "…"` parsed against entity names + aliases) — user picks per-chat. §7.4b enqueues one utterance per segment with the matched voice; the per-call swap from §7.4a is already in place. Optional polish: §7.5c (Preview button on the entity card) — now unblocked because per-call swap exists, just needs a Speaker entry point that synths a canned line through an arbitrary voice without touching the live playback queue.
