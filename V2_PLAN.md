...
- **§7.5d — Per-chat attribution mode picker — shipped 2026-05-05.** Chat header gains an "Attribution:" picker between the Server section and the Voice section, two items (Heuristic / Tagged), bound to `Chat.attributionMode`. The picker uses `representedObject` to carry the rawValue, so the action handler is decoupled from menu order — adding a third mode in §13 only needs an enum case + a `displayName` entry. `AttributionMode.displayName` lives in `SpeakerAttribution.swift` next to the enum so future modes get their UI label as a single addition. No new helper module yet — the picker is small enough that inlining it in `ChatViewController` (rebuild + action) costs less than abstracting. If a fourth voice-related per-chat picker shows up, fold all three (Voice, Attribution, future) into a shared chat-header builder. UI smoke-tested; the model-layer round-trip (`Chat.attributionMode` Codable migration) is already covered by `ChatSettingsVoiceTests`.
- **§7.5c — Preview button on the entity card — shipped 2026-05-05.** Entity card Voice row gains a "Preview" button to the right of the picker, only rendered when the entity has a voice set (sentinel "(use chat default)" → no voice → no button to mash). Click → `Speaker.preview(voice:)` synths a sample line through the chosen voice with the entity's rate/pitch applied. Disabled with a tooltip when the subsystem (`Settings.voiceEnabled`) is off, mirroring the chat-header speaker button. New `Speaker.preview(voice:)` honours the subsystem gate but bypasses the runtime mute (`voiceActive`) — the user explicitly pushed the button, the runtime mute is for streamed turns. Preview also stops any in-flight playback first so layered audio isn't possible. Sample text comes from `Speaker.previewText(for:)`: Kokoro voices use the catalogue's per-language pangram (`af_alloy` → English, `ff_siwis` → French, etc.); AVKit and uncatalogued Kokoro fall back to the English pangram (we don't parse AVKit voice ids for language so Speaker stays free of `AVFoundation` lookups, and a non-English AVKit voice still demonstrates its character via accent). 10 new pure tests in `SpeakerTests` cover the routing, gating, stop-before-preview, options carry-through, and per-language sample selection.

**Effort: ~2 days for §7.2–§7.5 once §7.1 lands.** §7.1, §7.2 (a/b/c), §7.5 (a/b/c/d), §7.3, §7.4 (a/b) all closed 2026-05-05 — Phase 6 is feature-complete for per-character voices, plus the attribution-mode escape hatch and the per-voice preview button. The heuristic refinement pass (was §7.6) has been deferred to **§13** — research item, design doc first, picked up after Phase 7 / Phase 8 (or sooner if attribution quality becomes the limiting factor in actual use).
...
Phases 1, 2, 3 are done. **Phase 6 is feature-complete** as of 2026-05-05 — §7.1, §7.2 (a/b/c), §7.5 (a/b/c/d), §7.3, §7.4 (a/b) all shipped, plus a heuristic-polish round (bare `I` short-circuit, lookback scoping, lenient first-person resolver, dialogue-verb subject detection). Per-character voices route end-to-end: pick a voice on an entity (with a Preview button to audition it), send a message, the matching quoted lines narrate in that voice via the heuristic attribution. Tagged mode (`Sage: "…"`) also works for chats where the model produces that convention, and the chat-header attribution-mode picker (§7.5d) lets the user switch per-chat. 477/477 tests pass.

From a fresh context, the pragmatic next move is **Phase 7 (V2 Full branching)** or **Phase 8 (V9 Group chats)** — both are sizeable refactors that warrant their own design rounds (PLAN.md §11). The heuristic refinement pass (was §7.6) has been deferred to **§13** — research item, design doc first, no concrete user friction yet to motivate it ahead of Phase 7 / 8. Pick it up if attribution quality becomes the limiting factor in real use, or as a palate-cleanser between the bigger refactors.
...
## 13. Deferred — Voice attribution heuristic refinement (was §7.6)

Moved out of Phase 6 on 2026-05-05 once the attribution-mode picker (§7.5d) gave users a runtime escape hatch (switch any chat to Tagged when the heuristic underperforms). Phase 6 ships without this; it's queued for after Phase 7 / Phase 8 unless attribution quality becomes the limiting factor in actual use.

**Why deferred:** the §7.3 polish round closed the obvious failure classes (quote-first dialogue verbs, bare `I` short-circuit, stale-mention dominance, character-card vs. entity-name mismatch). What remains is a long tail of model-specific quirks that needs *data*, not more speculative tuning. Without a corpus and an eval metric, further heuristic edits are sample-size-of-one tweaks that risk regressing earlier wins. The work is real, but it's research-shaped — better picked up deliberately than wedged between feature phases.

### 13.1 Background

The attribution heuristic — and the polish that landed 2026-05-05 — was tuned against a handful of one-off chat snippets the user fed back. Each round identified a class of mis-attribution and generalised it into a rule. That worked for the obvious classes, but the heuristic is necessarily fragile against the long tail of model output styles. A proper refinement pass should:
...
### 13.2 Agreed design decisions (recorded 2026-05-05)

These were settled in conversation before deferral. Honour them when the design doc is drafted; revisit only with explicit reason.

1. **Doc location.** Spin up a separate `V2_VOICE_HEURISTIC_REFINEMENT.md` at repo root. V2_PLAN.md is the index; the design doc cohabits with the code it affects and keeps the plan lean (the doc is expected to run 200–400 lines once it covers the corpus pipeline + labelling protocol + eval harness).
2. **MVP scope.** Hand-collected corpus only for v1. Defer the synthetic / self-driven-chat research point (see §13.3) to a follow-up. Spec the corpus format with a `source: "captured" | "synthetic"` field so a later synthetic pipeline can drop into the same eval harness without a schema change.
3. **Capture mechanism.** Both:
    - **Opt-in debug setting.** Flip in Settings; every assistant turn is recorded to disk until flipped off. Gets breadth — a representative sample of model styles over a few days of normal use.
    - **Per-turn "save to corpus" button.** On the turn UI, surfaces only when the debug setting is on or via a separate inspector affordance. Gets the obvious failures — turns the user noticed something off about. Biases the corpus toward failure cases, which is exactly what you want for refinement.
    A single `CorpusRecorder` (~30 lines) handles both; capture-point is the input boundary of `SpeakerAttribution.split` so the entity list, mode, and `firstPersonEntityId` are recorded alongside the raw text.
4. **Eval metric: per-character attribution accuracy.** Each character of the turn gets an attributed entity (or narrator) under the heuristic; compare char-by-char against the labelled entity for that character. The heuristic's job is "every word ends up routed to the right voice", and per-character accuracy is the most direct measurement of that. Robust to segment-boundary disagreements between heuristic and human labeller — boundary differences only matter when they straddle a quote, and per-character accuracy already accounts for that. (Per-segment exact match was considered and rejected: simpler intuition but penalises minor boundary differences that don't affect what the user hears.)

The design doc should propose: corpus on-disk JSON schema, `CorpusRecorder` placement + lifecycle, the labelling tool (CLI vs in-app — recommend CLI for v1), the eval harness (a `swift run AttributionEval <corpus.json>` that prints the confusion matrix + per-class precision/recall), and a labelling protocol (what counts as ground truth when a turn is genuinely ambiguous, e.g. a quote attributable to either of two characters present).

### 13.3 Research point — synthetic training data via self-driven chats
...
Per the agreed MVP scope above, this stays out of v1. Capture format should leave room for it (`source: "synthetic"` + a `generation_metadata` blob), but no code yet.
