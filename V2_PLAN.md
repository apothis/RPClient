...
**Effort: ~2 days for §7.2–§7.5 once §7.1 lands.** §7.1, §7.2 (a/b/c), §7.5 (a/b), §7.3, §7.4 (a/b) all closed 2026-05-05 — Phase 6 is feature-complete for per-character voices. Optional polish remaining: §7.5c (Preview button on the entity card, unblocked by §7.4a's per-call swap) and §7.5d (UI for picking attribution mode per chat).
...
Phases 1, 2, 3 are done. **Phase 6 is feature-complete** as of 2026-05-05 — §7.1, §7.2 (a/b/c), §7.5 (a/b), §7.3, §7.4 (a/b) all shipped. Per-character voices route end-to-end: pick a voice on an entity, send a message, the matching quoted lines narrate in that voice via the heuristic attribution. Tagged mode (`Sage: "…"`) also works for chats where the model produces that convention. 441/441 tests pass.

From a fresh context, the pragmatic next move is **Phase 7 (V2 Full branching)** or **Phase 8 (V9 Group chats)** — both are sizeable refactors that warrant their own design rounds (PLAN.md §11). Optional Phase 6 polish if anything feels rough in use:

- **§7.5c** — Preview button on the entity card. Now unblocked because §7.4a's per-call swap exists; just needs a `Speaker.preview(voice:)` entry point that synths a canned line through an arbitrary voice without touching the live playback queue.
- **§7.5d** — Per-chat attribution mode picker (small dropdown next to the chat-header voice picker, choices `Heuristic` / `Tagged`).
- **Long-form drift on heuristic.** §7.3 currently looks at *all preceding text* when matching mentions; long replies could drift onto whichever entity was named earliest. Revisit if it surfaces as a problem in practice — paragraph-scoped lookback would be the fix.
