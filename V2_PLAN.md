...
**Effort: ~2 days for §7.2–§7.5 once §7.1 lands.** §7.1, §7.2 (a/b/c), §7.5 (a/b) all closed 2026-05-05. Remaining order: §7.4 (queue + per-segment voice swap; unblocks Preview in §7.5a and is the load-bearing piece for actual per-character routing) → §7.3 (speaker attribution).
...
Phases 1, 2, 3 are done. Phase 6 §7.1, §7.2 (a/b/c), §7.5 (a/b) all closed 2026-05-05. Two-tier fallback shipped (`Entity.voice ?? Chat.voice ?? Settings.defaultVoice`); all three pickers exist in their respective surfaces and write through to the data model. **No audible per-character routing yet — the speaker layer still uses the single voice the §7.1l selector picked at chat-load.** 408/408 tests pass.

From a fresh context: pick up at **Phase 6 §7.4 (queue + per-segment voice swap)**. This is the load-bearing piece — refactor `KokoroSpeechSynthesizer` to swap style buffers per `speak(_:)` call instead of holding one at init (the engine + ONNX session stay shared; only the ~510 KB style embedding swaps), then refactor `Speaker` to enqueue `[(voiceId, text)]` segments and route each through the right voice. Once §7.4 lands, the picker UI from §7.5 starts producing audible behaviour and the Preview button on the entity card is unblocked.

After §7.4: §7.3 (speaker attribution — heuristic `^"…"` vs tagged `Sage: "…"`) lands last because attribution only matters once the queue can route segments to different voices.
