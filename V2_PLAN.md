...
f. **Two-tier toggle data layer (next up — TDD).** Add `Settings.voiceActive: Bool` (persisted, default `true`) with the same Codable migration pattern as `voiceModelPath`. Update `Speaker` to gate on `voiceEnabled && voiceActive` rather than just `voiceEnabled`. New `AppNotification.voiceActiveChanged` so the chat-header button stays in sync when the field is flipped from anywhere. Settings UI: relabel the existing checkbox to "Enable voice subsystem" — that's a one-line copy change. Pure data + a small `Speaker` revision; tests cover the gating combinations (both off / subsystem off + active on / subsystem on + active off / both on).

g. **First-run storage prompt sheet.** New `UI/Voice/VoiceStoragePromptSheet.swift` — fired when `voiceEnabled` (now the subsystem gate) flips on and `Settings.voiceModelPath` is nil. Lists `VoiceStorageScout.liveOptions()` rows (detected externals + Application Support) plus a "Choose…" escape hatch. Persists choice into `Settings.voiceModelPath`; dismissing the sheet falls back to Application Support. Subsequent toggles do not re-prompt.

h. **Settings UI — Voice library manager.** New `UI/Settings/VoiceLibraryView.swift`. Persistent catalogue table reading `KokoroVoiceCatalogue.all` and per-row state from `KokoroModelStore`. Filter chips (language, gender). Per-voice Download / Installed-Remove button + Preview button (enabled only when base + voice are installed). Storage-location picker (same NSOpenPanel as g). Base-model row with its own Download / Remove. No "download all" affordance — every voice is opt-in per voice. The whole section is hidden when `voiceEnabled == false`.

i. **Main-UI speaker toggle button.** New `UI/Voice/ChatHeaderSpeakerButton.swift` (or extension to the existing `ChatHeaderView` from Phase 4). NSButton with `speaker.wave.2` / `speaker.slash` SF Symbols swapping based on `Settings.voiceActive`. Disabled appearance + tooltip "Enable the voice subsystem in Settings" when `voiceEnabled == false`. Click toggles `voiceActive`; observes `voiceActiveChanged` + `settingsChanged` to refresh icon/enabled-state. Cheap to implement; pairs with f.

j. **Download manager.** Likely lands as `Voice/KokoroDownloadManager.swift` in `RPClientVoice` (since it has no Core consumers and writes ORT-relevant assets). One-shot URLSession download tasks per asset with progress reporting; concurrency cap of 2; cancellable mid-download. Computes SHA-256 on completion and calls `KokoroModelStore.recordModelDownloaded` / `recordVoiceDownloaded`. Pure-progress-tracking logic is testable with a stub URL session; URLSession glue is smoke-tested.
...
m. **Voice identifier namespacing.** §7.2's `voiceIdentifier` is stored as `engine:voice-id` (e.g. `kokoro:af_bella`, `avkit:com.apple.voice.premium.en-US.Ava`) so swapping engines later doesn't invalidate stored prefs. §7.5's voice picker is sourced from the catalogue, filtered to currently-installed voices.

n. **Smoke test.** `./build.sh && ./run.sh`, toggle "Enable voice subsystem" on in Settings (triggers first-run prompt → pick the external SSD), download base model + one voice, ensure the chat-header speaker button is enabled, click it on, send a message, confirm Kokoro output is materially better than the §7.0 AVKit baseline. Toggle the chat-header button off → confirm next reply is silent. Toggle subsystem off in Settings → confirm chat-header button greys out.
