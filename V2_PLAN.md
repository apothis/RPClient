...
**Status snapshot (2026-05-08).** Phases 1–8 shipped on `v2-plan`. Phase 9 §5.1 / §5.2 / §5.3 shipped 2026-05-06 → 2026-05-07 (design doc, full model + importer + exporter round-trip, creator window UI across 9 tabs, persistent custom tag vocabulary, RPClient-structured Details + Intimacy fields, drag-drop import-and-edit, Library entry points). **Phase 9 §5.4.0 (research) + §5.4.a (Mode 1 per-field suggestions) + §5.4.b (Mode 2 multi-field fill) shipped 2026-05-07 → 2026-05-08** — `CardSuggestionsController` triad strip on every multi-line field across 6 tabs with cross-tab stale propagation; "Fill missing fields" json_schema strict-mode dispatch with per-field Proposed-state UI + bulk Accept-all/Reject-all banner; greetings list-shaped single-shot Generate; novel-tag opt-in toggle (deferred to card-save); sex chooser on Identity tab plumbed as universal AI-assist upstream; identity-facts + tags universal upstream for narrative fields; tags field horizontal scrollability; bundled prompt registry with sharper structurally-distinct candidate styles + per-field overrides for list-shaped + greetings fields. **§5.4.c (Mode 3 full-card autopilot) is next.** 896/896 tests pass; Phase 9 work local until next push. **Phase 7's optional §3.6 (variant-collapse) deferred indefinitely.** The heuristic-refinement research item (§7) remains queued.
...
- **§5.2** — Card model expansion (`Character` field additions, schema migration if shape changes, importer + exporter updates, round-trip tests). Pure tests. ~1-2 days. **Shipped 2026-05-06** in five sub-passes (§5.2a JSONValue + v2-mappable Character fields `bd57959` → §5.2b v3 opt-in fields `d5986c9` → §5.2c importer v2/v3 + legacy-squash detect `9dd7acf` → §5.2d new `CharacterCardExporter` covering v2/v3 × JSON/PNG `5f398f1` → §5.2e round-trip integration tests on synthetic chub-shape NSFW + Risu v3 + assets + character_book + numeric-integrity + cross-format fixtures `782163d`). All additions decode-as-default backwards-compatible. v3 PNGs prefer `ccv3` chunk; fall back to `chara` for v2 cards or v3-via-backfill. v3 export writes both chunks for v2-reader compatibility. Stale `CharacterCardImporter` doc-comment fixed (claimed v1 rejection; never has). 687/687 green.
- **§5.3** — Card creator window UI (`NSWindowController` + per-field controls organised into tabs / sections, inline validation, examples-as-placeholder, save → `AppState.characters` list). Smoke-tested as a frontend layer. ~3-4 days. **Shipped 2026-05-07** in seven sub-passes (§5.3a window scaffold + Identity tab `709d6ed` + dark-mode/contrast/Library-cache regressions `232380f` + avatar diagnostic logging `23242a4` → §5.3b Persona/Greetings/Examples/System tabs `f7c4887` → §5.3c.1 persistent custom tags `18c2391` → §5.3c.2 Details + Intimacy tabs with body→build/anatomy/markings split + rich placeholders `0523925` → §5.3c.3 Lorebook tab read-only `045723e` → §5.3c.4 Advanced tab + extensions JSON viewer `88c6e03` → §5.3d Library entry points + drag-drop + Import & Edit menu `2af54d5`). The new design language landed under [`V2_DESIGN_LANGUAGE.md`](V2_DESIGN_LANGUAGE.md) as the foundation for the eventual V2_UI_OVERHAUL — the creator is the first surface that fully applies it. Several deferred polish items (in-place draft replacement on drag-drop, edit-tag-list UI, hide-the-fence on Description tab) catalogued in V2_PHASE9_CARD_CREATOR §5.3. 763/763 green.
- **§5.4** — AI-assist field generation, three modes per [`V2_PHASE9_AI_ASSIST_RESEARCH.md`](V2_PHASE9_AI_ASSIST_RESEARCH.md). Sub-step staging:
  - **§5.4.0** — research pass + 11 live empirical probes against the user's Qwen3.6 stack. **Shipped 2026-05-07** (`e6ddbc5`). Output is the research doc; settled the Qwen3 thinking-trap, free-form vs json_schema decision, KV-cache reuse layout, exemplar-set design, refusal-detection model-family patterns.
  - **§5.4.a** — Mode 1 per-field suggestions (3-candidate triad strip per multi-line field). **Shipped 2026-05-07 → 2026-05-08** in 6 slices + ~10 polish commits (95e3e74 → c9d7f4c, plus tags/spinner/echo-fix polish through 681241a). Strip wired into description / personality / scenario / first_message / message_example / system_prompt / post_history_instructions / creator_notes / appearance / mood + all 8 intimacy fields. Cross-tab stale propagation via `CardCreatorAIRegistry`. Greetings list-shaped single-shot Generate. Sex chooser on Identity tab as universal AI-assist upstream.
  - **§5.4.b** — Mode 2 multi-field fill ("Fill missing fields" button). **Shipped 2026-05-08** in 3 slices + polish (92ddced → c550aff). Single-call `response_format: json_schema` strict-mode against `/v1/chat/completions`; per-field Proposed-state UI with Accept/Reject/implicit-accept-on-edit; bulk Accept-all/Reject-all/Cancel banner; spinner during fetch.
  - **§5.4.c** — Mode 3 full-card autopilot. **Not started.** Single-seed → 5-6 sequential json_schema passes (Identity → Persona+Voice → Body+Intimacy → Disposition → System → Notes) → diff sheet with per-field lock + history + Re-roll. ~3-4 days.
  - **§5.4.d** — Polish + telemetry. **Not started.** Edit sheet for Mode 1 (currently aliases Use); single-line dropdowns for name/nickname/age/pronouns/species/orientation; "Fill this section" per-tab buttons; refusal-detection corpus from real smoke; lorebook editor + Generate (queued slice — needs prereq lorebook-edit UI first per V2_PHASE9_CARD_CREATOR §6).
...
**Status: scoping note shipped 2026-05-07; gated on Phase 9 §5.4 completion.** ~9-13 days when picked up.
...
**Deliverable:** `V2_UI_OVERHAUL.md` — a design doc that proposes the visual + interaction system before any code lands. Expected shape: principles → typography/spacing/colour system → per-surface mockups (sketches or annotated screenshots) → migration plan (which surfaces ship first, what risks). Likely runs longer than the other design docs (~500 lines) because it covers more surface area. **Foundation already drafted as [`V2_DESIGN_LANGUAGE.md`](V2_DESIGN_LANGUAGE.md) (2026-05-07)** — system-level principles, typography scale, spacing tokens, color, materials (macOS 26 Liquid Glass), motion, density posture, and the application contract for new surfaces. Phase 9 §5.3 (creator window) is the gold-standard reference implementation. The eventual overhaul doc inherits this foundation and builds the per-surface migration plan on top.
