...
| 4.b | TurnView re-skin | Rewrite [TurnView.swift](Sources/RPClientCore/UI/TurnView.swift) for the no-bubbles + 32pt avatar gutter layout (assistant). Speaker name + timestamp header. Italics-as-secondary tint. `<think>` collapsed disclosure. `✦` glyph migration to `person.crop.circle`. | High — most layout work lives here. | Pixel-stability snapshot for golden RP turns; test the `<think>` open/close height delta math. Glue tests OK for visual treatment. |
